...
func (s *S3SigningSuite) TestBuildSignedS3URLUSEast1() {
	...
	urlRegexp := fmt.Sprintf("^https://bucketname.s3-%s\\.amazonaws\\.com/path/file.json\\?X-Amz-Algorithm=AWS4-HMAC-SHA256&X-Amz-Credential=.*&X-Amz-Date=.*&X-Amz-Expires=%d&X-Amz-SignedHeaders=host&X-Amz-Signature=.*$", VALID_S3_REGION_US_EAST_1, VALID_S3_EXPIRY)
...
func (s *S3SigningSuite) TestBuildSignedS3URLUSWest2() {
	...
	urlRegexp := fmt.Sprintf("^https://bucketname.s3-%s\\.amazonaws\\.com/path/file.json\\?X-Amz-Algorithm=AWS4-HMAC-SHA256&X-Amz-Credential=.*&X-Amz-Date=.*&X-Amz-Expires=%d&X-Amz-SignedHeaders=host&X-Amz-Signature=.*$", VALID_S3_REGION_US_WEST_2, VALID_S3_EXPIRY)
	assert.Regexp(s.T(), urlRegexp, signedURL)
}
